...
def cores_da_bandeira(database):
  listacores = []
  if database[pais_sorteado]['bandeira']['vermelha']>0:
    listacores.append('vermelha')
  if database[pais_sorteado]['bandeira']['amarela']>0:
    listacores.append('amarela')
  if database[pais_sorteado]['bandeira']['laranja']>0:
    listacores.append('laranja')
  if database[pais_sorteado]['bandeira']['verde']>0:
    listacores.append('verde')
  if database[pais_sorteado]['bandeira']['azul']>0:
    listacores.append('azul')
  if database[pais_sorteado]['bandeira']['azul claro']>0:
    listacores.append('azul claro')
  if database[pais_sorteado]['bandeira']['preta']>0:
    listacores.append('preta')
  if database[pais_sorteado]['bandeira']['branca']>0:
    listacores.append('branca')
  return listacores
def acha_area(basededados):
  area = basededados[pais_sorteado]['area']
  return area

import random  
def sorteia_letra(palavra, lista):
    dim = palavra.lower()
    sort = random.choice(dim)
    listaprob = ['.', ',', '-', ';', ' ']
    if dim == '':
        return ''
    for letra in lista:
        listaprob.append(letra)
    for letrap in palavra:
        if letrap in listaprob:
            dim = dim.replace(letrap, '')
    if dim == '':
        return ''
    x = random.choice(dim)

    while x in listaprob:
        x = random.choice(dim)
    return x
...
def acha_populacao(data):
    pop = data[pais_sorteado]['populacao']
    return pop

def dic_dos_continentes(dicio):
  diciofinal = {}  
  chave = []
  for continente, dados in dicio.items():
    for pais, qualquercoisa in dados.items():
      diciofinal[pais] = continente
